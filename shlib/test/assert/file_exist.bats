...
@test "assert.file_exist: broken symlink" {
	source potpourri.shlib.assert.file_exist.bash

	shlib.assert.file_exist test/fixtures/assert/file_exist/foo2.txt

	[[ $status == 1 ]]
	[[ $output == *"assert.file_exist failed: File '$PWD/test/fixtures/assert/file_exist/foo2.txt' not found!" ]]
}

@test "assert.file_exist: real dir" {
	source potpourri.shlib.assert.file_exist.bash
...
@test "assert.file_exist: nonexistent file" {
	source potpourri.shlib.assert.file_exist.bash
